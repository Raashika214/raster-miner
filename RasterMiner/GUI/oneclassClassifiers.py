...
def showFinalTopKSamples(df):
    print("Final top- K  testing samples retrieved based on the Ravi Distance:")
    print(df)


def saveFinalTestSamples(df, outputFile):
    df.iloc[:, -1].to_csv(outputFile, header=True)


def saveFinalTopKSamples(df, outputFile):
    df.iloc[:, -1].to_csv(outputFile+ str("/TopKSamples.csv"), header=True)
...
def getStatistics(startTime):
    ...
    print("Memory used by proposedAlgo (in KB):", memory_in_KB)  # in bytes